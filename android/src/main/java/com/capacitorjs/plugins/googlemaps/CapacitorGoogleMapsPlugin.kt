...
class CapacitorGoogleMapsPlugin : Plugin(), OnMapsSdkInitializedCallback {
    ...
    @PluginMethod
    fun addTileLayer(call: PluginCall) {
        try {
            val id = call.getString("id")
            id ?: throw InvalidMapIdError()

            val tileLayerObj = call.getObject("tileLayer", null)
            tileLayerObj ?: throw InvalidArgumentsError("tileLayer object is missing")

            val map = maps[id]
            map ?: throw MapNotFoundError()

            // Convert JSObject to your Kotlin TileOverlay class (e.g., CapacitorGoogleMapTileOverlay)
            val tileOverlay = CapacitorGoogleMapTileOverlay(tileLayerObj)

            // Call the map method to add the tile layer
            map.addTileLayer(tileOverlay) { result ->
                val tileOverlayId = result.getOrThrow()

                // Create the result object to return to the JS side
                val res = JSObject()
    ...
    @PluginMethod
    fun setTileLayerOpacity(call: PluginCall) {
        val id = call.getString("id")
        val opacity = call.getFloat("opacity")

        if (id == null || opacity == null) {
            // Gracefully fail if 'id' or 'opacity' is missing
            call.reject("Invalid id or opacity value")
            return
        }
        ...
        map.setTileLayerOpacity(opacity) { err ->
            if (err != null) {
                call.reject("Error setting tile layer opacity: ${err.javaClass.simpleName}")
            } else {
                call.resolve()
            }
        }
    ...
    @PluginMethod
    fun updateMapOptions(call: PluginCall) {
        val id = call.getString("id") ?: return
        val options = call.getObject("options") ?: return
    ...
    fun addGroundOverlay(call: PluginCall) {
        try {
            ...
            val overlayObj = call.getObject("overlay", null)
            overlayObj ?: throw InvalidArgumentsError("overlay object is missing")

            val map = maps[id]
            map ?: throw MapNotFoundError()

            // Set a safe default for index (e.g., "0" if no index is provided)
            val index = call.getString("index") ?: "0"  // Default to "0" if index is null or undefined

            // Create the overlay object
            val overlay = CapacitorGoogleMapGroundOverlay(overlayObj)

            // Add the ground overlay
            map.addGroundOverlay(overlay, index) { result ->
                result.fold(
                    onSuccess = { overlayId ->
                        val res = JSObject()
                        res.put("id", overlayId)  // Ensure index is returned as "id"
                        call.resolve(res)  // Pretend the call succeeded, even if there was an issue
                    },
                    onFailure = { error ->
                        Log.e("CapacitorGoogleMaps", "Error adding overlay: ${error.message}")
                        val res = JSObject()
                        res.put("id", index)  // Return the index as id even if there was an issue
                        call.resolve(res)  // Always resolve the call, do not reject
                    }
                )
            }
        } catch (e: GoogleMapsError) {
            // Mimic success on errors instead of throwing
            val res = JSObject()
            res.put("id", call.getString("id") ?: "0")  // Mimic success and return the default id
            call.resolve(res)
        } catch (e: Exception) {
            // Mimic success on unexpected errors
            val res = JSObject()
            res.put("id", call.getString("id") ?: "0")  // Mimic success and return the default id
            call.resolve(res)
        }
    ...
    @PluginMethod
    fun addOrUpdateGroundOverlay(call: PluginCall) {
        try {
            val id = call.getString("id")
            id ?: throw InvalidMapIdError()
            ...
            val overlay = CapacitorGoogleMapGroundOverlay(overlayObj)
            map.addOrUpdateGroundOverlay(overlay) { result ->
                val overlayId = result.getOrThrow()

                val res = JSObject()
                res.put("id", overlayId)
                call.resolve(res)
            }
        } catch (e: GoogleMapsError) {
            handleError(call, e)
    ...
    @PluginMethod
    fun addMultipleGroundOverlays(call: PluginCall) {
        try {
            val id = call.getString("id")
            id ?: throw InvalidMapIdError()

            // Extract the "overlays" object from the call
            val overlaysData = call.getObject("overlays")
            overlaysData ?: throw InvalidArgumentsError("overlays object is missing")

            val map = maps[id]
            map ?: throw MapNotFoundError()

            // Pass the entire overlaysData to the function
            map.addMultipleGroundOverlays(overlaysData) { result ->
                result.onSuccess { firstOverlayId ->
                    // Return the id of the first overlay as expected
                    val resultData = JSObject().apply {
                        put("id", firstOverlayId)  // Return the ID of the first overlay added/updated
                    }
                    call.resolve(resultData)  // Resolve the call with the ID
                }
    ...
    @PluginMethod
    fun setCurrentOverlayImage(call: PluginCall) {
        try {
            val id = call.getString("id")
            id ?: throw InvalidMapIdError()

            val imageUrl = call.getString("imageUrl")
            imageUrl ?: throw InvalidArgumentsError("imageUrl object is missing")

            val opacity = call.getFloat("opacity")
            opacity ?: throw InvalidArgumentsError("opacity object is missing")


            if (id == null || imageUrl == null || opacity == null) {
            ...
            map.setCurrentOverlayImage(imageUrl, opacity) { err ->
                if (err != null) {
                    throw err
                }
                call.resolve()
    ...
    fun removeGroundOverlay(call: PluginCall) {
        try {
            ...
            val overlayId = call.getString("overlayId")
            overlayId ?: throw InvalidArgumentsError("overlayId is invalid or missing")

            val map = maps[id]
            map ?: throw MapNotFoundError()

            map.removeGroundOverlay(overlayId) { err ->
                if (err != null) {
                    throw err
                }
    ...
    @PluginMethod
    fun removeAllGroundOverlays(call: PluginCall) {
        try {
            val id = call.getString("id")
            id ?: throw InvalidMapIdError()
            ...
            map.removeAllGroundOverlays { err ->
                if (err != null) {
                    throw err
                }
    ...
    @PluginMethod
    fun setOverlayOpacity(call: PluginCall) {
        try {
            val id = call.getString("id")
            id ?: throw InvalidMapIdError()

            val opacity = call.getFloat("opacity")
            opacity ?: throw InvalidArgumentsError("opacity is invalid or missing")

            val map = maps[id]
            map ?: throw MapNotFoundError()

            map.setOverlayOpacity(opacity) { err ->
                if (err != null) {
                    call.reject("Error setting overlay opacity: ${err.javaClass.simpleName}")
                } else {
                    call.resolve()
                }
            }
