...
export class GoogleMap {
   ...
   */
  async updateMapOptions(options: MapOptions): Promise<void> {
    await CapacitorGoogleMaps.updateMapOptions({
      id: this.id,
      options,
    });
  }
   ...
   *
   * @param overlay
   * @returns created ground overlay id
   */
  ...
  async addTileLayer(tileLayer: TileOverlay): Promise<string> {
    const res = await CapacitorGoogleMaps.addTileLayer({
      id: this.id,
      tileLayer,
    });
  ...
  async setTileLayerOpacity(opacity: number): Promise<void> {
    return CapacitorGoogleMaps.setTileLayerOpacity({
      id: this.id,
      opacity
    });
  }

  async addOrUpdateGroundOverlay(overlay: GroundOverlay): Promise<string> {
    const res = await CapacitorGoogleMaps.addOrUpdateGroundOverlay({
      id: this.id,
      overlay,
    });
  ...
  async addMultipleGroundOverlays(overlays: MultipleGroundOverlays): Promise<string> {
    const res = await CapacitorGoogleMaps.addMultipleGroundOverlays({
      id: this.id,
      overlays,
    });

    return res?.id; // Only returning one ID, similar to the previous behavior
  }

  /**
   * Sets the opacity of the current ground overlay.
   *
   * @param opacity The opacity value (0.0 - 1.0).
   */
  async setOverlayOpacity(opacity: number): Promise<void> {
    await CapacitorGoogleMaps.setOverlayOpacity({
      id: this.id,
      opacity,
    });
  }
   ...
   *
   * @param id id of the ground overlay to remove from the map
   * @returns
   */
  async removeGroundOverlay(id: string): Promise<void> {
    return CapacitorGoogleMaps.removeGroundOverlay({
      id: this.id,
      overlayId: id,
    });
  }
  ...
  /**
   * Remove all ground overlays from the map
   */
  async removeAllGroundOverlays(): Promise<void> {
    return CapacitorGoogleMaps.removeAllGroundOverlays({
      id: this.id,
    });
  }

  async setCurrentOverlayImage(imageUrl: string, opacity: number): Promise<void> {
    return CapacitorGoogleMaps.setCurrentOverlayImage({
      id: this.id,
      imageUrl,
      opacity,
    });
